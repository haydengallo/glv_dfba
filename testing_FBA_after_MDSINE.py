...
sub_1948_init_mets.columns = ['reaction','fluxValue','upper_bound']
...
sub_1948_init_mets['fluxValue'] =  -1.0*(np.double(sub_1948_init_mets['fluxValue']))
...
for i in range(0, len(bi_hourly_resolution_latent_traj.columns)-1):
  ...
  if i == 0:
    base_array = np.linspace(bi_hourly_resolution_latent_traj.iloc[:,i], bi_hourly_resolution_latent_traj.iloc[:,i+1], 5)
  else:
    temp_array = np.linspace(bi_hourly_resolution_latent_traj.iloc[:,i], bi_hourly_resolution_latent_traj.iloc[:,i+1], 5)[1:,:]
...
cobra_models_dir = Path('/Users/haydengallo/UMass_Dropbox/Dropbox (UMass Medical School)/Bucci_Lab/glv_FBA/gLV_FBA_test_Kennedy_et_al_2025/processed_data_filtered_RC_all_cohorts_corrected_abs_abun/translatedDraftReconstructions')
...
genome_assembly_out_dir = os.path.join(processed_data, 'genomes')
...
models_for_FBA['18673193aa6bf30c6a1e71ac504e04df'].optimize()
...
met_pool_over_time, model_abun_dict = static_dfba(list_model_names=model_names, list_models=models_list, initial_abundance=init_abun, total_sim_time=800, num_t_steps=800, glv_out=np.array(abun_hr_df_filt.T), glv_params=None, environ_cond=sub_1948_init_mets, pfba=True, MDSINE_rates=rate_df_filt, Diet=RC_diet, time_points_feed = feeding_schedule)
...
met_save = '/Users/haydengallo/UMass_Dropbox/Dropbox (UMass Medical School)/Bucci_Lab/glv_FBA/gLV_FBA_test_Kennedy_et_al_2025/processed_data_filtered_RC_all_cohorts_corrected_abs_abun/' + str(test_num) + 'met_pool.npy'
abun_save = '/Users/haydengallo/UMass_Dropbox/Dropbox (UMass Medical School)/Bucci_Lab/glv_FBA/gLV_FBA_test_Kennedy_et_al_2025/processed_data_filtered_RC_all_cohorts_corrected_abs_abun/' + str(test_num) + 'abun_save.npy'
...
output_folder = 'filtering_daily_resolution'
...
change_met_ids = bigg_to_agora_exchange_ids(metabolomics_data_sub_2000_ra.index.tolist())
