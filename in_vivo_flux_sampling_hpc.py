...
for key in subject_dict.keys():
    ...
    ### Here calculate the stable t points 
    perturbations = np.load('perturbations_50_samps_08_05_2026.npy')
    interactions = np.load('interactions_50_samps_08_05_2026.npy')
    ...
    ### filter metabolomics data to grab the diet
    rc_diet_data = metabolomics_data[metabolomics_data['SampleName'] == 'RC_001']
    ### make a copy to have uncorrupted copy available
    ...
    ### adjust rc diet by adding max and min flux plus changing up column naming 
    ...
    ### Making dict of initial conditions for current subject and RC dietary conditions too 
    ...
    ### create rate array from the abundance data for constraint later
    rate_array = np.zeros((15,bi_hourly_resolution_latent_traj.shape[1]-1))
    for i in range(0,(len(bi_hourly_resolution_latent_traj.columns)-1)):

        rate_array[:,i] = (bi_hourly_resolution_latent_traj.iloc[:,i+1]/bi_hourly_resolution_latent_traj.iloc[:,i])-1

    ### make rate array a df
...
#######################################
### Individual strain abun overtime ###
#######################################
...
###################################################
### RA of metabolites over time vs experimental ###
###################################################
...
#############################################
### Subplots of each metabolite over time ###
#############################################
...
######################################################################
### Now here start to get the dataframes in order for scatterplots ###
######################################################################
...
##############################################
### Metabolites through time scatterplots  ###
##############################################
...
############################################################
### Metabolites through time scatterplots with log scale ###
############################################################
